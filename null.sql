...
/*5. Use COALESCE to print the mobile number. Use the number '07986 444 2266' if there is no
...
SELECT COUNT(name), COUNT(CASE WHEN COALESCE(mobile) THEN mobile END) FROM teacher;
...
SELECT COALESCE(dept.name), COUNT(teacher.dept) FROM teacher RIGHT JOIN dept ON (dept.id=teacher.dept)
GROUP BY dept.name;
